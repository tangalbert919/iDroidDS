...
void GPUEngineBase::_Reset_Base()
{
	...
	renderState.colorEffect = this->_IORegisterMap->BLDCNT.ColorEffect;
...
FORCEINLINE long
GPUEngineBase::_ColorEffectBlend(const u16 colA, const u16 colB, const u16 blendEVA, const u16 blendEVB)
{
	u16 ra =  colA        & 0x001F;
...
FORCEINLINE void GPUEngineBase::_PixelUnknownEffect(GPUEngineCompositorInfo &compInfo, const u16 srcColor16, const u8 spriteAlpha, const bool enableColorEffect)
{
	...
	TBlendTable const *selectedBlendTable = (TBlendTable const *) compInfo.renderState.blendTable555;
	...
	if ((LAYERTYPE == GPULayerType_OBJ) && enableColorEffect)
	{
		...
		if (isObjTranslucentType && dstEffectEnable)
		{
			...
			if (spriteAlpha != 0xFF)
			{
				...
				selectedBlendTable = (TBlendTable const *) &GPUEngineBase::_blendTable555[blendEVA][blendEVB];
			}
...
void GPUEngineBase::_SpriteRenderPerform(GPUEngineCompositorInfo &compInfo, u16 *__restrict dst, u8 *__restrict dst_alpha, u8 *__restrict typeTab, u8 *__restrict prioTab)
{
	...
	for (size_t i = 0; i < 128; i++)
	{
		...
		if (spriteInfo.RotScale != 0)
		{
			...
			dx  = LE_TO_LOCAL_16(this->_oamList[blockparameter+0].attr3);
			dmx = LE_TO_LOCAL_16(this->_oamList[blockparameter+1].attr3);
			dy  = LE_TO_LOCAL_16(this->_oamList[blockparameter+2].attr3);
			dmy = LE_TO_LOCAL_16(this->_oamList[blockparameter+3].attr3);
...
void GPUEngineBase::ParseReg_BLDCNT()
{
	...
	renderState.colorEffect = BLDCNT.ColorEffect;
