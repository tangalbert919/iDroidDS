...
public:
  virtual bool handleError(Error err, const char* message, CodeEmitter* origin) override {
    ASMJIT_UNUSED(err);
...
Error CodeBuilder::runPasses() {
  ...
  if (ASMJIT_UNLIKELY(err)) return err;
  ...
  if (logger != nullptr) {
    if (!logger->hasOption(Logger::kOptionDebugPasses))
      logger = nullptr;
  }
#else
  ...
  _passZone.reset();
  if (ASMJIT_LIKELY(!err))
    return err;

  const char* msg = nullptr;
  if (!postponed._message.isEmpty())
    msg = postponed._message.getData();
  return setLastError(err, msg);
}
